...
async def on_message(message):
    ...
    print("Message author:", author_id)
...
@bot.command()
async def refresh(ctx): #kind of like a refresh command
    channel = ctx.channel

    # 1. Update player list with all the current people on the server

    # 2. Get the most recent messages
    async for message in channel.history(limit=7):
        if message.author.id == 487622829748125696 and "streak" in message.content.lower():
            ...
            for i in range(len(lines)):
                ...
                if guess_number_index >= 0:
                    ...
                    for j in range(guess_number_index + 1, len(spaced_line)):
                        ...
                        leaderboard_db.create_score(id_buffer, guess_number, formatted_date)
                        print(spaced_line[j])

            await message.channel.send("Added scores to the leaderboards!")

# Read previous 100 texts
...
async def duplicate(ctx):
    ...
    for i in range(5):
        print(i)
        leaderboard_db.sample_duplicate()

    await ctx.send("Duplicate has been made")
